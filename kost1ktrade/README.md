...
Этот скрипт отвечает за сбор данных, их обработку и обучение модели машинного обучения, которая используется `MasterController` для принятия решений.

-   **Запустите обучение** (из директории `kost1ktrade/backend`):
    ```bash
    pipenv run python scripts/train_model.py
    ```
-   Скрипт по умолчанию обучает модель на данных BTC/USDT за последние 2 года. Вы можете настроить параметры, используя флаги:
    ```bash
    # Пример: обучить модель для ETH/USDT за последние 6 месяцев
    pipenv run python scripts/train_model.py --symbol "ETH/USDT" --start_date "2025-03-01" --end_date "2025-08-31"
    ```

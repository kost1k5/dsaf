...
class PurgedTimeSeriesSplit():
    """
    A robust implementation of Purged and Embargoed Time-Series Cross-Validation.
    This method creates sequential splits suitable for financial time series,
    where the training set expands and the test set slides forward.

    - Purging: Removes training samples whose labels depend on information that
      is also part of the test set. This prevents lookahead bias.
    - Embargo: Adds a gap after each test set to prevent leakage from serial
      auto-correlation between the test set and the subsequent training set.
    """
    ...
    def split(self, X: pd.DataFrame, y: pd.Series = None, event_end_times: pd.Series = None):
        ...
        # Create fold boundaries, similar to how TimeSeriesSplit works internally
        # We create n_splits + 1 boundaries to define n_splits test sets
        ...
        for i in range(self.n_splits):
            ...
            # The training set ends an 'embargo' period before the test set begins

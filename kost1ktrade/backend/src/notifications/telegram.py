...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Welcome! Use /status, /backtest, /optimize, or /walkforward.")

async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    status_text = f"Signal Bot: *{bot_state.signal_bot_mode}*\nGrid Bot: *{bot_state.grid_bot_mode}*"
    await update.message.reply_text(status_text, parse_mode='Markdown')

async def _run_backtest_and_report(context: ContextTypes.DEFAULT_TYPE):
    job = context.job
    chat_id, strategy_class, symbol, timeframe = job.data
    try:
        collector = DataCollector()
        candles_list = collector.fetch_candles(symbol, timeframe, limit=365)
        if not candles_list:
            await context.bot.send_message(chat_id, "Failed to fetch data for backtest.")
            return
        ...
        results = run_backtest(strategy=strategy, data=candles_df)
        report_lines = [f"*Backtest Report for {strategy_class.__name__}*...", f"Sharpe: *{results['sharpe_ratio']:.2f}*, P/L: *{results['pnl_percent']:.2f}%*"]
        ...
        await context.bot.send_message(chat_id, f"An error occurred during backtest: {e}")

async def backtest_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Starting backtest...")
    chat_id = update.effective_chat.id
    context.job_queue.run_once(_run_backtest_and_report, 0, data=(chat_id, SmaCrossoverStrategy, 'BTC/USDT', '1d'), name=f"backtest_{chat_id}")
...
async def _run_optimization_and_report(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        candles_list = collector.fetch_candles(symbol, timeframe, limit=365)
        if not candles_list:
            await context.bot.send_message(chat_id, "Failed to fetch data for optimization.")
            return
        candles_df = pd.DataFrame(candles_list, columns=['open_time', 'open', 'high', 'low', 'close', 'volume'])
        candles_df['open_time'] = pd.to_datetime(candles_df['open_time'], unit='ms', utc=True)
        candles_df.attrs = {'symbol': symbol, 'timeframe': timeframe}
        ...
        await context.bot.send_message(chat_id, f"An error occurred during optimization: {e}")
...
async def optimize_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat_id = update.effective_chat.id
    context.job_queue.run_once(_run_optimization_and_report, 0, data=(chat_id, SmaCrossoverStrategy, 'BTC/USDT', '1d'), name=f"optimize_{chat_id}")
...
async def _run_walk_forward_and_report(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not candles_list:
            await context.bot.send_message(chat_id, "Failed to fetch data for Walk-Forward analysis.")
            return
        candles_df = pd.DataFrame(candles_list, columns=['open_time', 'open', 'high', 'low', 'close', 'volume'])
        candles_df['open_time'] = pd.to_datetime(candles_df['open_time'], unit='ms', utc=True)
        candles_df.attrs = {'symbol': symbol, 'timeframe': timeframe}
        ...
        await context.bot.send_message(chat_id, f"An error occurred during Walk-Forward analysis: {e}")

async def walkforward_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Starting Walk-Forward analysis...")
    chat_id = update.effective_chat.id
    context.job_queue.run_once(_run_walk_forward_and_report, 0, data=(chat_id, SmaCrossoverStrategy, 'BTC/USDT', '1d'), name=f"walkforward_{chat_id}")
